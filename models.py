...
for i, (img_id, breed) in enumerate(
        labels.loc[labels['rank'] == 2, ['id', 'breed']].values):
    ax = grid[i]
...
print('ResNet running time: {} Seconds'.format(end - start))
...
print('VGG train bottleneck features shape: {} size: {:,}'.format(
    train_vgg_bf.shape, train_vgg_bf.size))
print('VGG valid bottleneck features shape: {} size: {:,}'.format(
    valid_vgg_bf.shape, valid_vgg_bf.size))
...
print(
    'Validation VGG Accuracy {}'.format(
        accuracy_score(
            (yv *
             range(NUM_CLASSES)).sum(
                axis=1),
            valid_preds)))
end = time.clock()
print('VGG running time: {} Seconds'.format(end - start))
...
print(
    'Xception train bottleneck features shape: {} size: {:,}'.format(
        train_x_bf.shape,
        train_x_bf.size))
print(
    'Xception valid bottleneck features shape: {} size: {:,}'.format(
        valid_x_bf.shape,
        valid_x_bf.size))
...
print(
    'Validation Xception Accuracy {}'.format(
        accuracy_score(
            (yv *
             range(NUM_CLASSES)).sum(
                axis=1),
...
print('Xception running time: {} Seconds'.format(end - start))
...
Xv = x_train[valid_idx]
print((Xtr.shape, Xv.shape, ytr.shape, yv.shape))
inception_bottleneck = inception_v3.InceptionV3(
...
print(
    'InceptionV3 train bottleneck features shape: {} size: {:,}'.format(
        train_i_bf.shape,
        train_i_bf.size))
print(
    'InceptionV3 valid bottleneck features shape: {} size: {:,}'.format(
        valid_i_bf.shape,
        valid_i_bf.size))
...
print(
    'Validation Inception Accuracy {}'.format(
        accuracy_score(
            (yv *
             range(NUM_CLASSES)).sum(
                axis=1),
...
print('Inception running time: {} Seconds'.format(end - start))
...
print('Validation Xception + Inception Accuracy {}'.format(accuracy_score((yv *
                                                                           range(NUM_CLASSES)).sum(axis=1), valid_preds)))
...
all_end = dt.datetime.now()
print('Total time: {} s.'.format((all_end - all_start).seconds))
